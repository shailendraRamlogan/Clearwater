  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const result = await createBooking({
        ...
        guests: store.guests.slice(1).filter((g) => g.first_name || g.last_name || g.email),
      });
      ...
      if (!clientSecret) {
        ...
        const cParams = new URLSearchParams({
          ...
          total: String(store.getSubtotal()),
        });
                ...
                {store.totalGuests() > 1 && (
                  <div className="space-y-2 pt-2 border-t border-white/10">
                    <p className="text-xs text-white/40">
                      Additional guests {store.missingGuestCount() > 0 && `(${store.missingGuestCount()} details to collect later)`}
                    </p>
                    {store.guests.slice(1).map((g, i) => (
                      <div key={i + 1} className="relative rounded-lg bg-white/5 border border-white/10 p-2">
                        <div className="flex items-center gap-2 mb-1.5">
                          <span className="text-[10px] font-medium text-white/40">Guest {i + 2}</span>
                          <button
                            onClick={() => store.removeGuest(i + 1)}
                        ...
                        <div className="flex gap-2">
                          <input
                            placeholder="First name"
                            value={g.first_name}
                            onChange={(e) => store.updateGuest(i + 1, "first_name", e.target.value)}
                            className="flex-1 h-8 px-2 rounded bg-white/10 border border-white/5 text-white text-xs placeholder-white/20 outline-none"
                          />
                          <input
                            placeholder="Last name"
                            value={g.last_name}
                            onChange={(e) => store.updateGuest(i + 1, "last_name", e.target.value)}
                            className="flex-1 h-8 px-2 rounded bg-white/10 border border-white/5 text-white text-xs placeholder-white/20 outline-none"
                          />
                        </div>
                        <input
                          ...
                          value={g.email}
                          onChange={(e) => store.updateGuest(i + 1, "email", e.target.value)}
                          className="w-full h-8 px-2 mt-1.5 rounded bg-white/10 border border-white/5 text-white text-xs placeholder-white/20 outline-none"
                        />
                      </div>
                    ))}
                    {store.guests.length < store.totalGuests() && (
                      <button
                        onClick={() => store.addGuest()}
                        className="w-full h-8 rounded-lg border border-dashed border-white/15 text-white/40 text-xs hover:text-white/60 hover:border-white/25 flex items-center justify-center gap-1"
                      >
                        <UserPlus className="w-3 h-3" /> Add guest details
                      </button>
                    )}
                  </div>
                )}
...
function GuestSlots({ store }: { store: PrivateTourState }) {
  const needed = store.totalPartySize() - 1;
  const [slots, setSlots] = useState(store.guests);

  useEffect(() => {
    const current = usePrivateTourStore.getState().guests;
    if (current.length < needed) {
      ...
      usePrivateTourStore.setState({ guests: [...current, ...additions] });
    }
  }, [needed]);

  useEffect(() => {
    setSlots(usePrivateTourStore.getState().guests);
    const unsub = usePrivateTourStore.subscribe((s) => setSlots(s.guests));
    return unsub;
  }, []);

  return (
    <>
      {slots.slice(0, needed).map((g, i) => (
        <div key={g.id} className="rounded-lg bg-white/5 border border-white/10 p-2 space-y-1.5">
          <span className="text-[10px] font-medium text-white/40">Guest {i + 2}</span>
          <div className="flex gap-2">
            <input
              placeholder="First name"
              value={g.first_name}
              onChange={(e) => store.updateGuest(g.id, "first_name", e.target.value)}
              className="flex-1 h-8 px-2 rounded bg-white/10 border border-white/5 text-white text-xs placeholder-white/20 outline-none"
            />
            <input
              placeholder="Last name"
              value={g.last_name}
              onChange={(e) => store.updateGuest(g.id, "last_name", e.target.value)}
              className="flex-1 h-8 px-2 rounded bg-white/10 border border-white/5 text-white text-xs placeholder-white/20 outline-none"
            />
          </div>
          <input
            ...
            placeholder="Email (optional)"
            value={g.email}
            onChange={(e) => store.updateGuest(g.id, "email", e.target.value)}
            className="w-full h-8 px-2 rounded bg-white/10 border border-white/5 text-white text-xs placeholder-white/20 outline-none"
          />
        </div>
      ))}
    </>
          ...
          <div className="px-5 py-4 max-h-[55vh] overflow-y-auto scrollbar-hide">
            ...
            {step === 4 && (
              <div className="space-y-3">
                ...
                {store.totalPartySize() > 1 && (
                  <div className="space-y-2 pt-2 border-t border-white/10">
                    <p className="text-xs text-white/40">Other guests (optional)</p>
                    <GuestSlots store={store} />
                  </div>
                )}
