              ...
              <div className="space-y-3">
                ...
                {store.totalGuests() > 1 && (
                  <div className="pt-4 mt-4 border-t border-brand-100">
                    <p className="text-sm font-medium text-brand-700 mb-3">Additional guests</p>
                    {store.guests.slice(1).map((g, i) => (
                      <div key={i + 1} className="rounded-xl border border-brand-100 p-3 mb-2">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs font-medium text-brand-500">Guest {i + 2}</span>
                          <button onClick={() => store.removeGuest(i + 1)} className="text-brand-300 hover:text-red-400">
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        </div>
                        <div className="flex gap-2">
                          <input placeholder="First" value={g.first_name} onChange={(e) => store.updateGuest(i + 1, "first_name", e.target.value)}
                            className="flex-1 h-9 px-3 rounded-lg border border-brand-200 text-sm text-brand-900 outline-none" />
                          <input placeholder="Last" value={g.last_name} onChange={(e) => store.updateGuest(i + 1, "last_name", e.target.value)}
                            className="flex-1 h-9 px-3 rounded-lg border border-brand-200 text-sm text-brand-900 outline-none" />
                        </div>
                        <input type="email" placeholder="Email (optional)" value={g.email} onChange={(e) => store.updateGuest(i + 1, "email", e.target.value)}
                          className="w-full h-9 px-3 mt-2 rounded-lg border border-brand-200 text-sm text-brand-900 outline-none" />
                      </div>
                    ))}
                    {store.guests.length < store.totalGuests() && (
                      <button onClick={() => store.addGuest()} className="w-full h-10 rounded-lg border border-dashed border-brand-200 text-brand-400 text-sm hover:text-brand-600 flex items-center justify-center gap-1">
                        <UserPlus className="w-4 h-4" /> Add guest
                      </button>
                    )}
                    {store.missingGuestCount() > 0 && (
                      <p className="text-xs text-amber-600 mt-2">⚠ {store.missingGuestCount()} guest detail{store.missingGuestCount() !== 1 ? "s" : ""} to be collected later</p>
                    )}
                  </div>
                )}
